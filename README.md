...
### 1. 게임 로직 (Game Logic)
- **라이어 선정 & 제시어**: 게임 시작 시 랜덤으로 라이어와 주제/제시어가 선정됩니다. 라이어에게는 가짜 제시어가 보입니다.
- **턴 기반 발언 시스템**: 
  - 플레이어 순서가 무작위로 섞입니다 (`Fisher-Yates Shuffle`).
  - 현재 턴인 플레이어에게만 **"공식 발언 입력 모달"**이 뜹니다.
  - 모든 플레이어의 발언이 끝나야 다음 라운드/투표로 넘어갈 수 있습니다.
- **라운드 설정**: 호스트가 최대 8라운드까지 설정 가능합니다.
- **투표 시스템**: 라운드 종료 후 투표를 통해 라이어를 지목합니다. 최다 득표자는 최후의 변론 기회를 얻습니다.
- **승리 조건**:
  - 시민 승리: 라이어를 맞추고, 라이어가 제시어를 맞추지 못했을 때.
  - 라이어 승리: 시민이 엄한 사람을 지목하거나, 라이어가 정답을 맞췄을 때.

### 2. 사용자 인터페이스 (UI/UX)
- **심플 & 다크 테마**: 블랙/화이트/그린 포인트 컬러를 사용한 직관적인 디자인.
- **반응형 디자인**: 모바일 환경에서도 완벽하게 동작합니다.
- **시각적 효과**: 
  - 발언 순서를 타임라인 형태로 시각화.
  - 팝업(Modal)을 활용한 깔끔한 정보 제공 (기록 보기, 발언 입력).

### 3. 기술적 특징
- **Firebase Realtime Database**: 실시간 데이터 동기화로 딜레이 없는 멀티플레이 경험 제공.
- **안전한 환경변수 관리**: API 키 보안 처리 완료.
- **GitHub Pages 배포**: 별도의 백엔드 서버 없이 정적 호스팅으로 구동.

## 🕹️ 게임 방법 (How to Play)

1. **방 만들기**: 이름을 입력하고 '방 만들기'를 누릅니다.
2. **초대하기**: 생성된 **방 ID**를 친구들에게 공유합니다.
3. **게임 시작**: 친구들이 모두 들어오면 호스트가 게임을 시작합니다.
4. **발언 진행**:
   - 자신의 차례가 되면 화면 중앙에 **입력창**이 뜹니다.
     - *(라이어는 정체를 들키지 않게 거짓말을 하세요!)*
   - 다른 사람의 턴일 때는 경청합니다.
   - `📝 지난 기록 보기` 버튼으로 이전 발언들을 다시 볼 수 있습니다.
5. **투표 및 결과**:
   - 모든 라운드가 끝나면 투표를 진행합니다.
   - 라이어로 지목된 사람은 마지막으로 제시어를 맞출 기회가 주어집니다.
